window.addEventListener("load", function(){
    ...
    getData("equipments.json", equipmentsList, "список оборудования для проверки", draw_select);
    ...
    function draw_table(ob1, val){
        //проверяем весь файл checklists
        for (let i = 0; i < ob1.length; i++){
            //если id оборудования совпадает с выбранным в списке
            if (Number(ob1[i].equipment_id) === Number(val) ){

                //формирование шапки таблицы проверки
                createTableRow(checkTable, "th", null, "№", "id", "Вопрос", "Выполнено?");
    ...
    function draw_select(ob){
        ...
        for (let i = 0; i < ob.length; i++){
            //добавление наименований оборудования в список
        ...
        .then((response) => {
            if (response.ok) {
              return response.json();
                ...
                throw new Error(`Не удалось загрузить ${errorDescription}: ${response.status} ${response.statusText}`);
            }
          })
